...
Fields marked with [`->multiple()`](/importers/fields#-multiplebool-multiple--true) -- and `MultiChoice` fields, which are inherently multi -- treat each cell as a comma-separated list of values. Each item is validated independently:

- **`Email`/`Phone` with `->multiple()`:** The type's default rule (`email` or `phone:AUTO`) is applied to every comma-separated item.
- **`MultiChoice` with `->options([...])`:** Each item must match one of the predefined options (case-insensitive).
- **`MultiChoice` without options:** Each item is validated through the field's `->rules()`. Use `Rule::exists(...)` to validate items against a database table, for example.

When any item fails, the review step surfaces a per-item error so users can correct individual values rather than the entire cell. The raw cell value is never split during storage -- consumers that need an array (for example, a JSON column) can explode the string inside `prepareForSave()`:

```php [app/Importers/CustomerImporter.php]
public function prepareForSave(array $data, ?Model $existing, array &$context): array
{
    if (isset($data['emails']) && is_string($data['emails'])) {
        $data['emails'] = array_values(array_filter(array_map(
            'trim',
            explode(',', $data['emails']),
        )));
    }
