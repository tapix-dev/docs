...
4. **Per-row processing** -- For each row: `beforeFill` > `prepareForSave()` > `afterFill` > `beforeSave` > `beforeCreate`/`beforeUpdate` > save > `afterCreate`/`afterUpdate` > `afterSave`.
5. **Post-import** -- `afterImport()` runs once after all rows are processed.

Hooks come in two styles: **method hooks** (traditional methods you override) and **convention-based hooks** (parameterless methods called via `callHook()`).
...
::warning
The `$data` parameter in `afterSave` receives the import context array (containing keys like `tenant_id` and `creator_id`), not the original row data. To access row data, use the model's attributes directly via `$model->getAttributes()`. This differs from `beforeSave`, which receives the original row data.
::
...
In addition to the method hooks above, Tapix supports convention-based hooks called via `callHook()`. These are parameterless methods on your importer that are called at specific points during row processing. They access row state through the properties set on the importer instance.

### Available State

Inside convention-based hooks, the following properties are available:

| Property | Type | Description |
|----------|------|-------------|
| `$this->data` | `array` | The current row data. |
| `$this->record` | `?Model` | The model instance (null before creation). |
| `$this->context` | `array` | Import context with `tenant_id` and `creator_id`. |
| `$this->isCreating` | `bool` | Whether the current row is being created (vs. updated). |

### Hook Execution Order

For each row, convention-based hooks fire in this order:

1. **`beforeFill`** -- Before data is assigned. `$this->data` contains the raw mapped data.
2. **`afterFill`** -- After `prepareForSave()` runs and data is prepared, before the save decision.
3. **`beforeSave`** -- Before the record is saved (runs for both create and update).
4. **`beforeCreate`** or **`beforeUpdate`** -- Create- or update-specific hook, runs after `beforeSave`.
5. **`afterCreate`** or **`afterUpdate`** -- Runs after the record is saved to the database.
6. **`afterSave`** -- Runs after `afterCreate`/`afterUpdate` (runs for both create and update).
...
final class ContactImporter extends BaseImporter
{
    ...
    {
        // Normalize email before any processing
    ...
    {
        $this->record->uuid = (string) Str::uuid();
    }

    public function afterCreate(): void
    {
        // Send welcome notification for newly imported contacts
    ...
    public function afterUpdate(): void
    {
        ...
        logger()->info("Contact {$this->record->id} updated via import");
    }
}
```
...
The `HasSkipSave` trait allows you to skip persisting records to the database. This is useful for dispatch-only importers that process rows without creating or updating models (e.g., triggering API calls or dispatching jobs per row).

Your importer must use the `HasSkipSave` trait:

```php [app/Importers/WebhookDispatchImporter.php]
use Tapix\Core\Importing\BaseImporter;
use Tapix\Core\Importing\Concerns\HasSkipSave;

final class WebhookDispatchImporter extends BaseImporter
{
    use HasSkipSave;

    protected bool $skipSave = true;
    ...
    public function afterFill(): void
    {
        ...
        Http::post('https://webhook.example.com', $this->data);
    }
}
```

When `$skipSave` is `true`, the row is counted as skipped and no model is persisted. The `beforeSave`, `beforeCreate`/`beforeUpdate`, `afterCreate`/`afterUpdate`, and `afterSave` hooks are not called.
