...
Controls whether the upload step shows a "Download Template" button for this importer. The template CSV uses field keys as headers and `->example()` values as a sample row. Defaults to the value of `config('tapix.example_csv.enabled')`, which is `true` by default.

Override to disable the template for a specific importer:
