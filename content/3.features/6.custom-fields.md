...
During `ExecuteImportJob`, custom field data is collected for each chunk of rows and flushed to the adapter at the end of each chunk by calling `saveValues()` once per record. This approach reduces the number of database operations compared to saving custom field values row by row.
