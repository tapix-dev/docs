...
|--------|---------|-------------|
| `queues.validation` | `sync` | Queue for column validation jobs. Runs synchronously by default. |
| `queues.execution` | `default` | Queue for the main import execution job. |
| `job_timeout` | `600` | Maximum execution time in seconds for each job. |
...
::warning
The `retry_after` value in your queue connection **must** exceed `job_timeout`. If it does not, the queue worker may restart a job that is still running, causing duplicate row processing. Set `retry_after` to at least `job_timeout + 30` seconds.
::

**Recommended setting:** `retry_after` >= `job_timeout` + 30 seconds.

For example, with the default `job_timeout` of 600 seconds, set `retry_after` to at least 630 in your `config/queue.php`:
