...
::note
The Livewire wizard requires the `tapix/livewire` package: `composer require tapix/livewire`
::
...
1. **Upload CSV** (STEP_UPLOAD) -- Upload a CSV or XLSX file for processing.
...
The wizard persists its state using a `?import=` query parameter containing a ULID. If a user navigates away and returns to the same URL, the wizard resumes from where they left off using the import record stored in the database.
